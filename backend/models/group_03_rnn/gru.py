...
class GRUModel(BaseModel):
    """GRU regressor for Group 3 sequence forecasting.
    ...
    def _reshape_input(self, X: np.ndarray) -> np.ndarray:
        """Validate and reshape the fixed Group 3 input schema."""
        if X.ndim != 2:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray | None = None) -> "GRUModel":
        """Train the GRU regression model."""
        if y is None:
            raise ValueError("GRUModel is supervised; y must not be None.")
        ...
        gru_layer.to(self._device)
        head.to(self._device)

        loss_fn = nn.MSELoss()

        optimizer = torch.optim.Adam(
            list(gru_layer.parameters()) + list(head.parameters()),
            ...
            if current_loss + self.min_delta < best_loss:
                ...
                best_state = {
                    "gru": {
                        ...
                        for key, value in gru_layer.state_dict().items()
                    },
                    "head": {
                        key: value.detach().cpu().clone()
        ...
        if best_state is not None:
            gru_layer.load_state_dict(best_state["gru"])
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self._model is None:
            raise RuntimeError(
                "Internal model state is unavailable; call fit() again."
            )
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray | None:
        """Return None because GRUModel is a regressor."""
        if not self.is_fitted:
    ...
    def get_metadata(self) -> dict:
        """Return metadata required by the integration contract."""
        return {
            "model_name": "GRU Regressor",
