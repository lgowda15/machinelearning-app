...
def make_fixture(seed=42, n_samples=160, n_features=6):
    ...
    lookback = GRUModel.EXPECTED_LOOKBACK
...
def test_fit_returns_self():
    X, y, _ = make_fixture()
    m = GRUModel(max_epochs=80, patience=8)
...
def test_predict_shape_and_proba_none():
    ...
    m = GRUModel(max_epochs=80, patience=8).fit(X, y)
...
def test_exactly_six_features_is_accepted():
    ...
    m = GRUModel(max_epochs=30, patience=5).fit(X, y)
    pred = m.predict(X_test)
    assert pred.shape == (40,)
...
def test_model_actually_trains_and_predictions_are_not_identical():
    X, y, X_test = make_fixture()
    m = GRUModel(max_epochs=120, patience=12).fit(X, y)
...
def test_feature_count_mismatch():
    X, y, _ = make_fixture()
    m = GRUModel(max_epochs=40, patience=6).fit(X, y)
...
def test_non_finite_values_rejected():
    ...
    with pytest.raises(ValueError, match="non-finite"):
        GRUModel().fit(X_nan, y)

    y_inf = y.copy()
    y_inf[0] = np.inf
    with pytest.raises(ValueError, match="non-finite"):
        GRUModel().fit(X, y_inf)


def test_metadata_keys_and_type():
    X, y, _ = make_fixture()
    m = GRUModel(max_epochs=50, patience=6).fit(X, y)
    md = m.get_metadata()
    assert set(md.keys()) == {
        "model_name",
        "model_type",
...
def test_deterministic_training_predictions():
    X, y, X_test = make_fixture()
    a = GRUModel(max_epochs=80, patience=8).fit(X, y).predict(X_test)
    b = GRUModel(max_epochs=80, patience=8).fit(X, y).predict(X_test)
