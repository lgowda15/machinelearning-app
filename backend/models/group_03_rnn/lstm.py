...
class LSTMModel(BaseModel):
    """Vanilla LSTM regressor for Group 3 sequence forecasting.
    ...
    def fit(self, X: np.ndarray, y: np.ndarray | None = None) -> "LSTMModel":
        if y is None:
            raise ValueError("LSTMModel is supervised; y must not be None.")
        ...
        lstm_layer.to(self._device)
        head.to(self._device)

        loss_fn = nn.MSELoss()
        optimizer = torch.optim.Adam(
            list(lstm_layer.parameters()) + list(head.parameters()),
            ...
            if current_loss + self.min_delta < best_loss:
                ...
                best_state = {
                    "lstm": {
                        ...
                        for k, v in lstm_layer.state_dict().items()
                    },
                    "head": {
                        k: v.detach().cpu().clone()
        ...
        self._model = {"lstm": lstm_layer, "head": head}
    ...
    def get_metadata(self) -> dict:
        return {
            "model_name": "Vanilla LSTM Regressor",
