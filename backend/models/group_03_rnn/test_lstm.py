...
def make_fixture(seed=42, n_samples=160, n_features=6):
    ...
    lookback = LSTMModel.EXPECTED_LOOKBACK
    ...
    y = (
        ...
        - 0.1 * X_seq[:, 1, 1]
        + 1.2 * X_seq[:, 2, 0]
        + 0.25 * np.tanh(X_seq[:, 0, 0] - X_seq[:, 2, 1])
    ).astype(float)
...
def test_fit_returns_self():
    X, y, _ = make_fixture()
    m = LSTMModel(max_epochs=80, patience=8)
...
def test_predict_shape_and_proba_none():
    ...
    m = LSTMModel(max_epochs=80, patience=8).fit(X, y)
...
def test_exactly_six_features_is_accepted():
    ...
    m = LSTMModel(max_epochs=30, patience=5).fit(X, y)
    pred = m.predict(X_test)
    assert pred.shape == (40,)
...
def test_model_actually_trains_and_predictions_are_not_identical():
    X, y, X_test = make_fixture()
    m = LSTMModel(max_epochs=120, patience=12).fit(X, y)
...
def test_feature_count_mismatch():
    X, y, _ = make_fixture()
    m = LSTMModel(max_epochs=40, patience=6).fit(X, y)
    bad = np.zeros((5, m.n_features + 2), dtype=np.float64)
...
def test_non_finite_values_rejected():
    ...
    with pytest.raises(ValueError, match="non-finite"):
        LSTMModel().fit(X_nan, y)

    y_inf = y.copy()
    y_inf[0] = np.inf
    with pytest.raises(ValueError, match="non-finite"):
        LSTMModel().fit(X, y_inf)


def test_metadata_keys_and_type():
    X, y, _ = make_fixture()
    m = LSTMModel(max_epochs=50, patience=6).fit(X, y)
    md = m.get_metadata()
    assert set(md.keys()) == {
        "model_name",
        "model_type",
...
def test_invalid_input_shape_handling_in_fit_and_predict():
    rng = np.random.default_rng(42)
    X_bad = rng.standard_normal(40).astype(np.float64)
...
def test_deterministic_training_predictions():
    X, y, X_test = make_fixture()
    a = LSTMModel(max_epochs=80, patience=8).fit(X, y).predict(X_test)
    b = LSTMModel(max_epochs=80, patience=8).fit(X, y).predict(X_test)
